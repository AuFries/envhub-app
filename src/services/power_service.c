...
static void *power_thread_fn(void *arg)
{
    ...
    if (fd < 0) {
        syslog(LOG_ERR, "power_service: open(%s) failed: %s",
               POWER_EVENT_DEV, strerror(errno));
        return NULL;
    }
...
bool power_service_init(void)
{
    ...
    if (!g_chip) {
        syslog(LOG_ERR, "power_service: gpiod_chip_open(%s) failed",
               POWER_GPIOCHIP);
        return false;
    }
    ...
    if (!g_off_line) {
        syslog(LOG_ERR, "power_service: gpiod_chip_get_line(%d) failed",
               POWER_OFF_LINE);
    ...
    if (gpiod_line_request_output(g_off_line, "envhub-soft-power-off", 0) < 0) {
        syslog(LOG_ERR, "power_service: gpiod_line_request_output failed: %s",
               strerror(errno));
        g_off_line = NULL;
        gpiod_chip_close(g_chip);
        g_chip = NULL;
    ...
    syslog(LOG_INFO, "power_service: OFF gpio initialized low on %s line %d",
           POWER_GPIOCHIP, POWER_OFF_LINE);
...
bool power_service_power_cut(void)
{
    ...
    if (gpiod_line_set_value(g_off_line, 1) < 0) {
        syslog(LOG_ERR, "power_service: gpiod_line_set_value(1) failed: %s",
               strerror(errno));
        return false;
    }
